...
//Foster Report Generate Section
$sql = "SELECT * FROM Foster";
$result = $conn->query($sql);

// File name
$filename = "foster_report.txt";

// Open file
$file = fopen($filename, "w");

// Write header
fwrite($file, "Foster Report\n\n");
fwrite($file, str_pad("FosterID", 20) . str_pad("StartDate", 20) . str_pad("EndDate", 25) . str_pad("Address", 30) . str_pad("PetID", 20) . str_pad("CaregiverID", 15) . "\n");
...
if($result->num_rows > 0){
    ...
    fwrite($file, 
        str_pad($row['FosterID'], 20) .
        str_pad($row['StartDate'], 20) .
        str_pad($row['EndDate'], 25) .
        str_pad($row['Address'], 30) .
        str_pad($row['PetID'], 20) .
        str_pad($row['CaregiverID'], 15) .
		"\n"
    );}
}else{
...
//Start Foter data delete section
...
<body>
  ...
  <div class="main">
    ...
    <div class="topbar">
	  ...
	  <?php echo "<a href='$filename' download style='color:white;border-radius:20px;padding:10px 40px;background: linear-gradient(to right, #667eea, #764ba2);'>Foster Report Generate</a>"; ?>
    </div>
	<br /><br />
